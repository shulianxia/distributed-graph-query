...
class Coordinator:
    ...
    def query_triangles_global(self):
        """分布式全图三角计数
        策略：
          1. 从所有 Worker 收集全图边列表
          2. 对每条边 (u,v)，向 u 和 v 所在 Worker 索取邻居列表
          3. Coordinator 本地求交集 → 无遗漏
        """
        ...
        for wid, info in items:
            ...
            if resp and resp["msg_type"] == MSG_RESULT_OK:
                for u, v in resp.get("payload", {}).get("edges", []):
                    ...
                    else: all_edges.add((v, u))
        logger.info(f"收集到 {len(all_edges)} 条全图边")
        if not all_edges:
            return {"error": "收集边失败"}

        triangles = set()
        # 2. 对每条边，向两端 Worker 索取邻居列表
        for (u, v) in sorted(all_edges):
            info_u = self._route(u)
            info_v = self._route(v)
            if not info_u or not info_v:
                continue
            # 获取 u 的邻居
            nu = set()
            resp = self._rpc(info_u, make_msg(MSG_QUERY_NLIST, "coord", {"node": u}))
            if resp and resp["msg_type"] == MSG_RESULT_OK:
                nu = set(resp["payload"].get("neighbors", []))
            # 获取 v 的邻居
            nv = set()
            resp = self._rpc(info_v, make_msg(MSG_QUERY_NLIST, "coord", {"node": v}))
            if resp and resp["msg_type"] == MSG_RESULT_OK:
                nv = set(resp["payload"].get("neighbors", []))
            # 共同邻居 = 三角形第三个顶点
            for w in sorted(nu & nv):
                if w != u and w != v:
                    tri = tuple(sorted((u, v, w)))
                    triangles.add(tri)

        logger.info(f"分布式三角计数完成: {len(triangles)} 个三角形")
