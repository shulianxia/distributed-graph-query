#!/usr/bin/env python3
"""
进程通信图查询系统 — Coordinator

基于 test3 预取架构：
- 短连接模式
- 预取全图三角计数（O(P)次RPC）
- 增加 MSG_QUERY_NODE_INFO 路由
"""
...
from protocol import (
    ...
    MSG_QUERY_TRIANGLE, MSG_QUERY_NODE_INFO,
    MSG_REGISTER, MSG_HEARTBEAT, MSG_HEARTBEAT_ACK,
    make_msg, pack_msg, recv_msg, send_msg, recv_or_none, get_partition, logger
)
        ...
        MSG_QUERY_COMMON: lambda msg: make_msg(MSG_RESULT_OK, "coord",
            ...
            MSG_QUERY_NODE_INFO: lambda msg: make_msg(MSG_RESULT_OK, "coord",
                self.query_node_info(
                    msg["payload"].get("node", msg["payload"].get("node_id", 0)))),
        }
        ...
        for i, u in enumerate(nodes):
            ...
            for v in nu:
                ...
                for w in nu & nv:
                    if w > v:
                        triangles.add((u, v, w))
        logger.info(f"全图三角计数完成: {len(triangles)} 个")
