#!/usr/bin/env python3
"""
test5 一键启动脚本 — 生成数据 → 启动 Coordinator → 启动 Workers → 打开 TUI

用法：
  python3 start_demo.py              # 默认 200 节点
  python3 start_demo.py 500 0.06     # 自定义规模
  python3 start_demo.py --no-ui      # 只启动服务，不打开 UI
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="test5 分布式图查询演示")
    ...
    # 5. 启动 UI 或等待
    if args.no_ui:
        print_info("UI 未启动，请在另一终端运行:")
        print_info(f"  python3 ui.py --coord-port {coord_port}")
        print()
        ...
        print_info("正在启动交互式 TUI...")
        print()
        try:
            subprocess.run([
                sys.executable, os.path.join(BASE, "ui.py"),
                "--coord-host", "127.0.0.1",
